...
function tokenizeWithCanonical(
  text: string,
  canonicalAnalysis?: KiwiCanonicalAnalysis
): Array<{ token: string; start: number; end: number }> {
  if (!canonicalAnalysis?.ready || canonicalAnalysis.tokens.length === 0) {
    return tokenizeWithRanges(text);
  }
  return canonicalAnalysis.tokens
    .map((token) => ({
      token: text.slice(token.range.start, token.range.end),
...
export function runEditTagger(
  ...
  const tokens = tokenizeWithCanonical(text, canonicalAnalysis);
  return tokens.map((t) => {
