...
DEFAULT_DATASET_DIR = Path("./data/edit_tagger_v2_dataset")
DEFAULT_OUTPUT_DIR = Path("./models/edit_tagger_v2_koelectra")
DEFAULT_MODEL_NAME = "monologg/koelectra-base-v3-discriminator"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="KoELECTRA 기반 EditTagger v2를 학습합니다.")
...
def compute_class_weights(rows: List[Dict[str, Any]], label2id: Dict[str, int]) -> torch.Tensor:
    counts = np.ones(len(label2id), dtype=np.float64)
    for row in rows:
        for label in row["labels"]:
            counts[label2id[label]] += 1.0
    ...
    def compute_metrics(eval_pred):
        ...
        for pred_row, gold_row in zip(predictions, labels):
            for pred_id, gold_id in zip(pred_row, gold_row):
                ...
                if gold_label == "OPEN_REPLACE":
                    open_gold += 1
                if pred_label == "OPEN_REPLACE":
                    open_pred += 1
                if gold_label == "OPEN_REPLACE" and pred_label == "OPEN_REPLACE":
                    open_tp += 1

                if gold_label == "SPACE_FIX":
                    space_gold += 1
                if pred_label == "SPACE_FIX":
                    space_pred += 1
                if gold_label == "SPACE_FIX" and pred_label == "SPACE_FIX":
                    space_tp += 1

        non_keep_precision = safe_div(non_keep_tp, non_keep_pred)
        non_keep_recall = safe_div(non_keep_tp, non_keep_gold)
        non_keep_f1 = safe_div(2 * non_keep_precision * non_keep_recall, non_keep_precision + non_keep_recall)

        open_precision = safe_div(open_tp, open_pred)
        open_recall = safe_div(open_tp, open_gold)
        open_f1 = safe_div(2 * open_precision * open_recall, open_precision + open_recall)

        space_precision = safe_div(space_tp, space_pred)
        space_recall = safe_div(space_tp, space_gold)
        space_f1 = safe_div(2 * space_precision * space_recall, space_precision + space_recall)
        ...
        return {
            ...
            "open_replace_precision": open_precision,
            "open_replace_recall": open_recall,
            "open_replace_f1": open_f1,
            "space_fix_precision": space_precision,
            "space_fix_recall": space_recall,
            "space_fix_f1": space_f1,
            "keep_false_positive_rate": safe_div(keep_fp, keep_gold),
        }
...
def main() -> None:
    ...
    class_weights = compute_class_weights(dataset["train"], label_config.label2id)
    ...
    training_kwargs = {
        ...
        "metric_for_best_model": "open_replace_recall",
    ...
    trainer_signature = inspect.signature(WeightedTokenTrainer.__init__)
    ...
    metrics = {
        ...
        "class_weights": {label: round(float(class_weights[idx].item()), 6) for label, idx in label_config.label2id.items()},
        "args": {
