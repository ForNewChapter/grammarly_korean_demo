...
async function refineTaggerWithOnnx(
  ...
  if (!session || tags.length === 0) return { tags, modelUsed: false };
...
async function runPipeline(text: string, cursor: number, mode: 'realtime' | 'inspect'): Promise<PipelineResult> {
  ...
  const spacingModelOut = await refineSpacingCandidatesWithOnnx(clause, profile, spacingCandidatesRaw);
...
self.onmessage = async (evt: MessageEvent<WorkerRequest>): Promise<void> => {
  ...
  try {
    ...
    if (msg.type === 'WARMUP') {
      await modelManager.warmup(msg.models);
      return;
    }
